# SKILL.md — Skill portable para video-intake-knowledge

> Esta skill es portable y funciona en cualquier entorno compatible con SKILL.md,
> incluyendo Hermes Agent, AGY, OpenCode, Claude Code, Codex, y otros.
...
Para Hermes: integración nativa con la memoria del host.
Para otros hosts: exportar paquete Markdown/JSON para importación manual.
...
- Límites de tamaño, duración, número de vídeos por trabajo, concurrencia,
  timeouts.
- Sanitización de rutas, nombres de archivos y metadatos.
...
ofrecer alternativas: adjuntar archivo local, proporcionar subtítulos, usar
...
El plugin Hermes está en `adapters/hermes/plugin.yaml`. Registra:

- Tools: `video_intake_inspect_source`, `video_intake_create_job`,
  `video_intake_get_job_status`, `video_intake_cancel_job`,
  `video_intake_get_artifacts`, `video_intake_extract_transcript`,
  `video_intake_extract_audio_context`, `video_intake_extract_visual_context`,
  `video_intake_list_memory_banks`, `video_intake_store_memory`,
  `video_intake_create_memory_bank`, `video_intake_analyze_build_options`,
  `video_intake_generate_asset_draft`

- Hooks: detección de mensajes con URL o adjunto de vídeo antes de que
  el modelo responda.

- Comandos manuales: `/video`, `/video status`, `/video cancel`,
  `/video artifacts`, `/video doctor`

### Instalación

```bash
# Copiar plugin a dir de plugins de Hermes
cp -r adapters/hermes ~/.hermes/plugins/video-intake-knowledge/

# O instalar con el script
cd video-intake-knowledge
...
Ver `adapters/hermes/config.example.yaml`.

## Integración con otros hosts
...
video-intake extract archivo.mp4 --select 6
```
...
# Tras la extracción, elige "3" para añadir a banco existente
# o "4" para crear banco nuevo
```
